...
while (True):
    ...
    if(("not" in u) or ("cannot" in u) or ("exit" in u) or ("terminate " in u) or ("dont" in u) or ("did not" in u)):
    ...
    elif(("2" in u) or ("two" in u) or ("to" in u) or ("too" in u)):
        ...
        while(True):
            ...
            if (("run" in p) or ("start" in p) or ("open" in p)) and ("chrome"in p):
                ...
                os.system("firefox")
            elif (("stop" in p) or ("close" in p) or ("terminate" in p) or ("exit" in p)) and("firefox"in p):                                         
                os.system("TASKKILL /IM chrome.exe /F")                
            elif (("open" in p) or ("start" in p)) and ("gmail" in p):
                os.system("chrome www.gmail.com")
            elif (("send"in p)) and (("mail" in p) or ("gmail" in p)):
                os.system("python mail.py")                
            elif (("run" in p)or("start" in p)or("play" in p)) and (("music" in p) or ("player"in p)):
                os.system("wmplayer")
            elif  (("open" in p) or ("start" in p)) and ("vscode" in p):
                os.system("code")
            elif(("run" in p) or ("execute" in p) or ("open" in p)) and (("notepad" in p) or ("editior"in p)):
                os.system("notepad")
            elif ("exit" in p) or ("out" in p) or ("terminate" in p):
                break                
            elif (("make" in p) or ("create" in p)) and (("folder"in p) or ("dir" in p)):
                print("Enter your requirements .....we r listiening .... : " , end='')
                pyttsx3.speak("\n\n Enter your requirements .we r listiening . \n")
                r = sr.Recognizer()
                with sr.Microphone() as source:
            ...
            if("1" in m):
                    ...
                    if ("1" in chb):
                        ...
                        while True:
                            ...
                            if ("1" in chb1):
                                ...
                                print("\n\t\t Enter the name of security_group : " , end='')
                                sec_n = input()
                                print("\\nt\t\t Enter your key_name : ",end='')
                                k_n = input()
                                cmd="""aws ec2 run-instances  --image-id ami-0a9d27a9f4f5c0efc    --instance-type t2.micro    --count 1   --subnet-id  subnet-b6c8dfde   --security-group {}_sg  --key-name {} """.format(k_n,sec_n)
                                output=sp.getstatusoutput(cmd)
                                sts()
                                ...
                                keye_name = input()
                                cmd=r"ssh -l ec2-user {0}  -i C:\Users\Admin\Downloads\{1}.pem sudo yum repolist".format(ip1,keye_name)
                                output=sp.getstatusoutput(cmd)
                                sts()
                                ...
                                output=sp.getstatusoutput(cmd)
                                sts()
                                print("\n\t\t\t Enter the Disk_name : ",end='')
                                disc_name= input()
                                cmd=r"ssh -l ec2-user {}	  -i C:\Users\Admin\Downloads\{}.pem sudo mkfs.ext4    /dev/xvd{}	".format(ip2,k_name,disc_name)
                                output=sp.getstatusoutput(cmd)
                                sts()

                                cmd=r"ssh -l ec2-user {}	  -i C:\Users\Admin\Downloads\{}.pem sudo mount /dev/xvd{}    /var/www/html".format(ip2,k_name,disc_name)
                                output=sp.getstatusoutput(cmd)
                                sts()
                            elif("8" in chb1):
                                ...
                                cmd=r'aws s3 cp  E:\user data   s3://{}/ --acl public-read'.format(s3)
                                output=sp.getstatusoutput(cmd)
                                sts()
                            elif("9" in chb1):
                                print("\n\t\t\tEnter the bucket name ",end= ' ')
                ...
                while(True):
                    ...
                    if("1" in ch5):
                        ...
                        size_d = int(input())
                        cmd='ssh -l root {} resize2fs /dev/mapper/{}-{} {}G'.format(ipp,vg,lv,size_d)
                        output=sp.getstatusoutput(cmd)
                        sts()
